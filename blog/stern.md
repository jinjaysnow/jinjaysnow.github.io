...
```
#### 信号与信号处理  
信号是提供处理异步事件机制的软件终端。信号有一个明确的生命周期：首先：产生信号；然后内核存储信号知道可以发送它；最后，一旦有空闲，内核会适当的处理信号。信号处理通常有三种情况：

1. **忽略信号**  不采取任何操作。信号SIGKILL和SIGSTOP不能忽略。
2. **捕获并处理信号**  内核会暂停该进程正在执行的代码，并跳转到先前注册过的函数。接下来进程执行该函数，执行完后再跳回先来捕获信号的地方继续执行。常见的有SIGINT和SIGTERM。
